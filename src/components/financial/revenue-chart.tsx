...
export function RevenueChart() {
  ...
  const totalExpensesPaid = Number(data.summary.totalExpensesPaid ?? 0)
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card size="sm" className="rounded-[3px] bg-white shadow-[0_1px_4px_rgba(0,0,0,0.06)]" data-testid="net-profit-card">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-xs uppercase tracking-wider font-medium text-mid">Lucro Liquido</CardTitle>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <Card className="rounded-[3px] shadow-[0_1px_4px_rgba(0,0,0,0.06)]">
          <CardHeader>
            <CardTitle className="text-[14px] font-medium text-[#2A2A2A]">Receita por Metodo de Pagamento</CardTitle>
          </CardHeader>
          <CardContent>
            {paymentMethodData.length === 0 ? (
              <p className="py-8 text-center text-sm text-mid">
                Sem dados de pagamento no periodo
              </p>
            ) : (
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
