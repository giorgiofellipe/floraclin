...
const AUTO_FILL_KEYS: Record<string, (ctx: { contactName?: string | null }) => string | undefined> = {
  patient_name: (ctx) => ctx.contactName ?? undefined,
}
...
export function TemplatePicker({
  ...
  function handleSelectTemplate(template: Template) {
    ...
    const ctx = { contactName }
    for (const v of mapping) {
      const fn = AUTO_FILL_KEYS[v.key]
      const value = fn?.(ctx)
      if (value) autoFilled[v.index] = value
    }
    ...
    if (allFilled) {
      ...
      onSelectTemplate(template.name, template.language, params)
      return
    }
