...
interface SendConsentSigningLinkProps {
  ...
  /** Resolved server-side to the active template of each type. */
  consentTypes?: string[]
  /** Exact templates to sign; used when there is no procedure. */
...
export function SendConsentSigningLink({
  ...
  const hasSelection = (consentTemplateIds?.length ?? consentTypes?.length ?? 0) > 0
    ...
    try {
      const res = await fetch('/api/consent/send-signing-link', {
        ...
        body: JSON.stringify({
          patientId,
          procedureRecordId,
          ...(consentTemplateIds ? { consentTemplateIds } : { consentTypes }),
          renderedContents,
        }),
      })
      if (!res.ok) {
