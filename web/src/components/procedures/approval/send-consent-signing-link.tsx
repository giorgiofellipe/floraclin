...
/** Mirrors the two bodies `POST /api/consent/send-signing-link` accepts. */
type Selection =
  | { procedureRecordId: string; consentTypes: string[]; consentTemplateIds?: never }
  | { consentTemplateIds: string[]; procedureRecordId?: never; consentTypes?: never }

type SendConsentSigningLinkProps = CommonProps & Selection
...
function selectionBody(props: Selection) {
  return props.consentTemplateIds
    ? { consentTemplateIds: props.consentTemplateIds }
    : { procedureRecordId: props.procedureRecordId, consentTypes: props.consentTypes }
}

export function SendConsentSigningLink(props: SendConsentSigningLinkProps) {
  const {
    patientId,
    patientName,
    patientPhone,
    ...
    label = 'Enviar termos e contrato para assinatura',
  } = props
  ...
  const hasSelection = (props.consentTemplateIds ?? props.consentTypes ?? []).length > 0
  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch('/api/consent/send-signing-link', {
        ...
        body: JSON.stringify({ patientId, ...selectionBody(props), renderedContents }),
      })
      if (!res.ok) {
  ...
  function getWhatsAppUrl() {
    ...
    const message = `Olá ${firstName}! Acesse o link abaixo para revisar e assinar seus termos: ${url}`
    ...
    <div className="flex flex-wrap items-center gap-2 animate-in fade-in duration-300 w-full overflow-hidden">
      ...
      {whatsAppUrl && whatsappApiEnabled ? (
        <div className="flex items-stretch">
          ...
          >
            {sendingApi ? <Loader2Icon className="size-3.5 animate-spin" /> : <WhatsAppIcon />}
            Enviar via WhatsApp
          </button>
          <DropdownMenu>
            <DropdownMenuTrigger
