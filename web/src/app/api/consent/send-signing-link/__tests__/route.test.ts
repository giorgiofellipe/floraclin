...
describe('POST /api/consent/send-signing-link', () => {
  it('creates a token for explicit template ids with no procedure', async () => {
  ...
  it('refuses a template id that is inactive or belongs to another tenant', async () => {
    vi.mocked(getTemplatesForToken).mockResolvedValue([{ id: TEMPLATE_A }] as never)
  ...
  it('still resolves consent types for the procedure flow', async () => {
  ...
  it('rejects a body with both or neither selector', async () => {
    ...
    const neither = await post({ patientId: PATIENT_ID })
