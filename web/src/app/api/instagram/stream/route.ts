...
export async function GET() {
  ...
  if (!settings?.instagram_enabled) {
    return NextResponse.json({ error: 'Instagram not enabled' }, { status: 400 })
  }

  const allowedRoles = (settings.instagram_allowed_roles as string[] | undefined) ?? ['owner']
  if (!allowedRoles.includes(ctx.role) && ctx.role !== 'owner') {
    return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
  }

  let lastEventId = await getLatestSseEventId(ctx.tenantId, 'instagram')
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const poll = async () => {
        try {
          const events = await pollSseEvents(ctx.tenantId, lastEventId, 'instagram')
